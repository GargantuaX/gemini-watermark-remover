...
- 支持停止当前检测或导出并取消剩余视频队列，不下载未完成结果；清理结束后可重新选择文件处理。
- 独立视频界面支持按顺序批量处理视频，显示每个文件的状态并自动下载（#109，感谢 @kavyp12）。
- 处理中新增视频会追加到队列，不替换当前视频；单个文件失败后继续处理剩余文件。空闲时选择单个文件仍使用手动导出。
- 随运行时资产附带 ONNX Runtime 1.26.0 完整许可证和上游第三方声明。
- 图片、视频去水印算法及 SDK 接口保持不变；批量功能仅在开源独立界面提供，官网保留单视频流程，官网批量适配不作为本次发版前置条件。

## 1.0.44 - 2026-09-22

- 修复视频导出时背景候选替换全体采样一致选中的水印，造成水印残留和多余黑点的问题（#150）。
- 保留现有混合轨道检测及尽力输出行为。本次限定视频修复不修改图片处理算法，也不代表所有视频清理瑕疵已解决。

## 1.0.43 - 2026-09-10

- 修复部分受支持的 2752×1536 Gemini 插画背景图片中水印被跳过或定位错位的问题。
- 减少受支持木柜样本的明显水印残留，同时保留木纹和水平边界。
- 保留现有内容保护与尽力输出行为；部分图片仍可能有细小水印轮廓。

## 1.0.42 - 2026-09-10

- 修复 #153：识别 2752×1536 图片中已确认的 48px 水印及 89px 右侧/底部边距，避免误处理其他背景区域。
- 新布局需要较强的原图正向证据，保留标准水印选择和 best-effort 输出。
- 初始选择被偏移或无关候选占用时，保留通过原图证据验证的 exact-48 候选，同时保留原有校准与保守候选。
- 修正两类已复现的内容误识别，避免在背景上制造黑洞或新的星形痕迹。
- 同一批 424 张上传图对照 1.0.41：418 张输出不变，六张变化输出的随机视觉复核均偏好新版。仍保留轻微残留和来源未知的历史观察，不宣称所有输入均已干净。
- 本次图片更新不包含 #150 和 #152 的视频修复。
