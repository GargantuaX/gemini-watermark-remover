...
已验收制品入库时，在 `release/evidence/v1.0.44-video-build.json` 保留构建提交与 CI 运行编号。视频门禁会逐项核对生产输入及包元数据与原构建相同、验证原 CI 成功，并下载原制品比较；最终 HEAD 仍须独立通过 CI。这样可以提交版本制品，而不把旧制品冒称为新提交重新构建的结果。
